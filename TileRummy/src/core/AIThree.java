...
public class AIThree implements Player{
	...
	public void play(ArrayList<Meld> table, Deck d, ArrayList<Player> plist) {
		...
		System.out.println("");
		System.out.println("------- Player Three's Turn -------");
		...
		if (!hasPlayedFirstTurn) {
				while (true) {
					...
					if(meld.getNumTiles() == 0) {
						System.out.println("Player Three Draws a Tile.");
						hand.addTile(d.draw());
						break;
					}
								...
								if (meld.getMeldType() == "RUN") {
									if(tableMeld.getMeld().get(0).getColor() == meld.getMeld().get(0).getColor()) {
										if(meld.getMeld().get(0).getRank() - tableMeld.getMeld().get((table.get(i).getMeld().size()) - 1).getRank() == 1) {
											...
											System.out.print("Player Three plays Meld: ");
											meld.displayMeld();
		...
		} else if(canPlayAllTiles(table)) {
			playAllTiles(table);
		} else {
			boolean leaveWhile = false;
			while (!leaveWhile) {
				boolean noHandLessThan3 = false;
				for(Player p: plist) {
					if(hand.getNumTiles() - p.handSize() < 4)
						noHandLessThan3 = true;
				}
				if(noHandLessThan3) {
					...
					playIndividualTiles(table, passInHand, false);
					leaveWhile = true;
									...
									if(m.getMeldType() == "RUN") {
										...
										for(int w = 0; w < m.getNumTiles(); w++) {
											...
											if(t.getRank() - m.getMeld().get(w).getRank() == -1) {
												m.getMeld().add(w, t);
												removeTiles.add(t);
												System.out.print("Player Three Plays Tile: ");
												playedTurn2 = true;
												t.displayTile();
												System.out.println("");
												nextTile = true;
	...
	public boolean canPlayAllTiles(ArrayList<Meld> table) {
		//WINNING IN ONE TURN /////////////////////////////////////////////////
		Hand hand2 = new Hand();
	...
	public void playAllTiles(ArrayList<Meld> table) {
		boolean playedTurn = false;
		...
		for(int mIndex = 0; mIndex < winInOneTurn.size(); mIndex++) {
			if(mIndex == winInOneTurn.size() - 1) { //PLAY ALL THE SINGLE TILES
				...
				playIndividualTiles(table, passHand, false);
	...
	public void playIndividualTiles(ArrayList< Meld > tble, Hand h, boolean sizeRun) {
		//Play Individual Tiles on Table if Possible
